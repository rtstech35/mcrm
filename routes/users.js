...
router.post("/", authenticateToken, checkPermission('users.create'), async (req, res) => {
  try {
...
router.get("/:id", authenticateToken, checkPermission('users.read'), async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(`
      SELECT u.id, u.username, u.full_name, u.email, u.phone, u.is_active, u.role_id, u.department_id, r.name as role_name, d.name as department_name FROM users u
      LEFT JOIN roles r ON u.role_id = r.id
      LEFT JOIN departments d ON u.department_id = d.id
      WHERE u.id = $1
    `, [id]);
...
router.put("/:id", authenticateToken, checkPermission('users.update'), async (req, res) => {
  try {
    const { id } = req.params;
    const { username, email, password, full_name, role_id, department_id, phone, is_active } = req.body;
    let query = `UPDATE users SET username = $1, email = $2, full_name = $3, role_id = $4, department_id = $5, phone = $6, is_active = $7, updated_at = CURRENT_TIMESTAMP`;
...
router.delete("/:id", authenticateToken, checkPermission('users.delete'), async (req, res) => {
  try {
    ...
    if (error.code === '23503') {
        return res.status(409).json({ success: false, error: 'Bu kullanıcı başka kayıtlarda kullanıldığı için silinemez. Önce pasif hale getirmeyi deneyin.' });
    }
